...
public class BSTImpl<T extends Comparable<T>> implements BST<T> {
	public static void main(String[] args) {
		...
		System.out.println(bst.isIgualBST(bst1, bst2));
	...
	public boolean isIgualBST(BSTImpl<T> bst1, BSTImpl<T> bst2) {
		return isIgualBST(bst1.getRoot(), bst2.getRoot());
	}

	private boolean isIgualBST(BTNode<T> bst1Node, BTNode<T> bst2Node) {
		if (bst1Node.isEmpty() && bst2Node.isEmpty()) return true;
		if (!bst1Node.isEmpty() && bst2Node.isEmpty()) return false;
		if (bst1Node.isEmpty() && !bst2Node.isEmpty()) return false;
		if (!bst1Node.isEmpty() && !bst2Node.isEmpty()) {
			if (bst1Node.equals(bst2Node)) {
				return isIgualBST(bst1Node.getLeft(), bst2Node.getLeft()) && isIgualBST(bst1Node.getRight(), bst2Node.getRight());			
			}
		}
		return false;
	}
	...
	private boolean isBinarySearchTree(BSTNode<T> node) {
		if (!node.isLeaf()) {
			...
			if (!node.getLeft().isEmpty() && !node.getRight().isEmpty()) {
				return isBinarySearchTree((BSTNode<T>) node.getLeft()) && isBinarySearchTree((BSTNode<T>) node.getRight());
			}
			
			else if (node.getLeft().isEmpty()) {
				return isBinarySearchTree((BSTNode<T>) node.getRight());
			}
			
			else if (node.getRight().isEmpty()) {
				return isBinarySearchTree((BSTNode<T>) node.getLeft());
			}
		}
		return true;
	}
